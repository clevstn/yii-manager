                            ...
                            <div class="form-upload-group">
                                <?php for ($i = 0; $i < $options['number']; $i++): ?>
                                    <div class="inline-block">
                                        <div class="form-upload-control" ng-click="triggerSelectImage('<?= $options["fileType"] ?>', '<?= $options["saveDirectory"] ?>', '<?= $options["pathPrefix"] ?>', '<?= $options["fileScenario"] ?>', '<?= $field ?>', <?= $i ?>, <?= $options['number'] ?>)">
                                            <div class="form-upload-item">
                                                <i ng-hide="formFileLink<?= $field . $i ?>" class="fa fa-file-image-o f-32 text-dark"></i>
                                                <span ng-show="formFileLink<?= $field . $i ?>" class="img-remove fa fa-remove" ng-click="removeFiles($event, '<?= $field ?>', <?= $i ?>)"></span>
