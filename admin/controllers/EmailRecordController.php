 ...
 * @since ym1.0
 */
class EmailRecordController extends \app\builder\common\CommonController
{
    /**
     * @var array
     */
    ...
    public function actionIndex()
    {
        ...
        $table->query = function () use ($queryParams) {
            ...
            if (!empty($queryParams)) {
                ...
                $query->filterWhere([
                    'and',
                    // 发件人筛选
                    ['=', 'send_user', isset($queryParams['send_user']) ? $queryParams['send_user'] : null],
                    // 关键词筛选
                    [
                        'or',
        ...
        // 筛选
        $query = AdminUser::query('username,email,mobile,id')->all();
        $data[0] = '【System】';
        array_walk($query, function ($item) use (&$data) {
            $data[$item['id']] = "【{$item['username']}】【{$item['email']}】【{$item['mobile']}】";
        });
        $table->toolbarFilter = [
            'columns' => [
                'keyword' => table_toolbar_filter_helper([
                    ...
                    'label' => '关键词',
                    'placeholder' => '请输入服务名称/收件人',
                ]),
                'send_user' => table_toolbar_filter_helper([
